...
$fb_grupos_add = NULL; // Initialize page object first

class cfb_grupos_add extends cfb_grupos {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["idfb_grupos"] != "") {
				...
				$this->setKey("idfb_grupos", $this->idfb_grupos->CurrentValue); // Set up key
			} else {
				$this->setKey("idfb_grupos", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "fb_gruposview.php")
	...
	function LoadDefaultValues() {
		...
		$this->super_id->CurrentValue = NULL;
		$this->super_id->OldValue = $this->super_id->CurrentValue;
	}
	...
	function ValidateForm() {
		...
		if ($this->getCurrentDetailTable() == "fb_posts" && $GLOBALS["fb_posts"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// super_id
		$this->super_id->SetDbValueDef($rsnew, $this->super_id->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$this->idfb_grupos->setDbValue($conn->Insert_ID());
			$rsnew['idfb_grupos'] = $this->idfb_grupos->DbValue;
		}

		// Add detail records
		if ($AddRow) {
			if ($this->getCurrentDetailTable() == "fb_posts" && $GLOBALS["fb_posts"]->DetailAdd) {
				$GLOBALS["fb_posts"]->id_grupo->setSessionValue($this->super_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["fb_posts_grid"])) $GLOBALS["fb_posts_grid"] = new cfb_posts_grid(); // get detail page object
				$AddRow = $GLOBALS["fb_posts_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["fb_posts"]->id_grupo->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			if ($sDetailTblVar == "fb_posts") {
				...
				if ($GLOBALS["fb_posts_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["fb_posts_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["fb_posts_grid"]->CurrentMode = "add";
					$GLOBALS["fb_posts_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($fb_grupos_add)) $fb_grupos_add = new cfb_grupos_add();

// Page init
$fb_grupos_add->Page_Init();

// Page main
$fb_grupos_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var fb_grupos_add = new ew_Page("fb_grupos_add");
fb_grupos_add.PageID = "add"; // Page ID
var EW_PAGE_ID = fb_grupos_add.PageID; // For backward compatibility

// Form object
var ffb_gruposadd = new ew_Form("ffb_gruposadd");

// Validate form
ffb_gruposadd.Validate = function(fobj) {
...
ffb_gruposadd.Form_CustomValidate =  
...
ffb_gruposadd.ValidateRequired = true;
<?php } else { ?>
ffb_gruposadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ffb_gruposadd.Lists["x_pais"] = {"LinkField":"x_idpaises","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","x_admin","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<h4 id="ewPageCaption" class="page-header ewTitle ewTableTitle"> <?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $fb_grupos->TableCaption() ?></h4>
...
<?php $fb_grupos_add->ShowPageHeader(); ?>
<?php
$fb_grupos_add->ShowMessage();
?>
<form name="ffb_gruposadd" id="ffb_gruposadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<input type="hidden" name="t" value="fb_grupos" />
<input type="hidden" name="a_add" id="a_add" value="A" />
<table id="tbl_fb_gruposadd" class="ewTable ewTableSeparate table table-striped ">
...
<?php if ($fb_grupos->getCurrentDetailTable() == "fb_posts" && $fb_posts->DetailAdd) { ?>
<br />
