...
class cnoticias_add extends cnoticias {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["idnoticias"] != "") {
				...
				$this->setKey("idnoticias", $this->idnoticias->CurrentValue); // Set up key
			} else {
				$this->setKey("idnoticias", ""); // Clear key
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("idnoticias")) <> "")
			$this->idnoticias->CurrentValue = $this->getKey("idnoticias"); // idnoticias
		else
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$this->idnoticias->setDbValue($conn->Insert_ID());
			$rsnew['idnoticias'] = $this->idnoticias->DbValue;
		}
		if ($AddRow) {
...
// Validate form
fnoticiasadd.Validate = function(fobj) {
...
<h4 id="ewPageCaption" class="page-header ewTitle ewTableTitle"> <?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $noticias->TableCaption() ?></h4>
...
?>
<form name="fnoticiasadd" id="fnoticiasadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<input type="hidden" name="t" value="noticias" />
<input type="hidden" name="a_add" id="a_add" value="A" />
<table id="tbl_noticiasadd" class="ewTable ewTableSeparate table table-striped ">
</table>
</div>
