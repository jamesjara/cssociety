...
class cpaises_delete extends cpaises {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate($this->james_url( "paiseslist.php" )); // Prevent SQL injection, return to list
...
// Page object
var paises_delete = new ew_Page("paises_delete");
paises_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = paises_delete.PageID; // For backward compatibility

// Form object
var fpaisesdelete = new ew_Form("fpaisesdelete");
...
fpaisesdelete.Lists["x_admin"] = {"LinkField":"x_id_owners","Ajax":true,"AutoFill":false,"DisplayFields":["x_Correo_Electronico","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($paises_delete->Recordset = $paises_delete->LoadRecordset())
	$paises_deleteTotalRecs = $paises_delete->Recordset->RecordCount(); // Get record count
if ($paises_deleteTotalRecs <= 0) { // No record found, exit
	if ($paises_delete->Recordset)
		$paises_delete->Recordset->Close();
	$paises_delete->Page_Terminate($this->james_url( "paiseslist.php" )); // Return to list
}
?>
<h4 id="ewPageCaption" class="page-header ewTitle ewTableTitle"> <?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $paises->TableCaption() ?></h4>
...
?>
<form name="fpaisesdelete" id="fpaisesdelete" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<?php } ?>
<br />
<table id="tbl_paisesdelete" class="ewTable ewTableSeparate table table-striped table-bordered ">
<?php echo $paises->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<th><span id="elh_paises_nombre" class="paises_nombre">
		<?php echo $paises->nombre->FldCaption() ?></span></th>
		<th><span id="elh_paises_admin" class="paises_admin">
		<?php echo $paises->admin->FldCaption() ?></span></th>
	</tr>
	</thead>
	<tbody>
<?php
...
?>
	<tr<?php echo $paises->RowAttributes() ?>>
		<td<?php echo $paises->nombre->CellAttributes() ?>><span id="el<?php echo $paises_delete->RowCnt ?>_paises_nombre" class="paises_nombre">
<span<?php echo $paises->nombre->ViewAttributes() ?>>
<?php echo $paises->nombre->ListViewValue() ?></span>
</span></td>
		<td<?php echo $paises->admin->CellAttributes() ?>><span id="el<?php echo $paises_delete->RowCnt ?>_paises_admin" class="paises_admin">
