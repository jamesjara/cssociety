...
function CurrentProjectID() {
	...
	return "{B9BF3307-7A7A-4ACA-81A3-DC180D98192D}";
}
...
class cAdvancedSecurity {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						$this->setCurrentUserName($rs->fields('Correo Electronico')); // Load user name
	...
	function CurrentUserEmail() {
		return $this->CurrentUserInfo("Correo Electronico");
	}
...
function ew_UploadPathEx($PhyPath, $DestPath) {
	if ($PhyPath) {
		...
		$Path = conf_cdn_photos.'/'.basename( $DestPath );//jamesjara
	}
...
class cUserProfile {
	...
	function SaveProfileToDatabase($usr) {
		...
		if ($usr == "" || $usr == EW_ADMIN_USER_NAME) // Ignore hard code admin
			return FALSE;
		$sFilter = str_replace("%u", ew_AdjustSql($usr), EW_USER_NAME_FILTER);
		$sSql = "UPDATE " . ew_QuotedName($owners->TableName) .
